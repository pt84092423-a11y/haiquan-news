...
function normalizeUrl(value, fallback = DEFAULT_IMG) {
  ...
  if (trimmed.startsWith('/')) return `https://${SITE_DOMAIN}${trimmed}`;
  return fallback;
}
...
function buildOgHtml(post, slug) {
  ...
  const description = escapeHtml(stripHtml(post.meta_description || post.excerpt || DEFAULT_DESC));
...
function injectOgIntoHtml(html, ogHtml) {
  const cleaned = html
    ...
    .replace(/<meta\s+(?:name|property)=["'](?:description|author|og:[^"']+|article:[^"']+|twitter:[^"']+)["'][^>]*>\s*/gi, '')
