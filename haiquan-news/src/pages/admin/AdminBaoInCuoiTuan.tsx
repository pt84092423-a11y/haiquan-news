                ...
                <div className="flex flex-col gap-1.5 max-h-[420px] overflow-y-auto pr-1">
                      ...
                      <button key={(post as any).id} onClick={() => !disabled && toggleSelect(post)} disabled={disabled}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className={`text-[10px] font-bold px-1.5 py-0 rounded-full ${post.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-600'}`}>
                  ...
                  {filteredPosts.length === 0 && !loading && (
                    <p className="text-[12px] text-gray-400 text-center py-4">
                      {allPosts.length === 0 ? 'Chưa có bài viết nào' : 'Không tìm thấy bài phù hợp'}
                    </p>
                  )}
                </div>
              )}
